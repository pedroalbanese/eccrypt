...
func Encrypt(pub *PublicKey, data []byte, random io.Reader,mode int) ([]byte, error) {
	...
	for {
		...
		y2Buf := y2.Bytes()
		if n := len(x1Buf); n < 20 {
			x1Buf = append(zeroByteSlice()[:20-n], x1Buf...)
		}
		if n := len(y1Buf); n < 20 {
			y1Buf = append(zeroByteSlice()[:20-n], y1Buf...)
		}
		if n := len(x2Buf); n < 20 {
			x2Buf = append(zeroByteSlice()[:20-n], x2Buf...)
		}
		if n := len(y2Buf); n < 20 {
			y2Buf = append(zeroByteSlice()[:20-n], y2Buf...)
		}
		c = append(c, x1Buf...)
		c = append(c, y1Buf...)
		tm := []byte{}
...
func kdf(length int, x ...[]byte) ([]byte, bool) {
	...
	for i, j := 0, (length+23)/20; i < j; i++ {
		...
		hash := h.Sum(nil)
		if i+1 == j && length%20 != 0 {
			c = append(c, hash[:length%20]...)
		} else {
			c = append(c, hash...)
		}
		ct++
